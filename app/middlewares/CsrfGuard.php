...
class CsrfGuard extends \Slim\Middleware
{
     ...
     * @param string    $key        The CSRF token header name.
     * @return void
     */
    public function __construct($key = 'csrf_token', $header = 'X-CSRF-Token')
    {
    ...
    public function check() {
        ...
        if (in_array($this->app->request()->getMethod(), array('POST', 'PUT', 'DELETE'))) {
            $userToken = $this->app->request()->post($this->key, $this->app->request->headers($this->header));
            if ($token !== $userToken) {
